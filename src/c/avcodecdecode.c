...
static void write_frame(struct xlplayer *xlplayer, AVFrame *frame)
    ...
    int buffer_size = av_samples_get_buffer_size(NULL, channels,
                        frame->nb_samples, self->c->sample_fmt, 1);

    switch (self->c->sample_fmt) {
        case AV_SAMPLE_FMT_FLT:
            frames = (buffer_size >> 2) / channels;
            memcpy(self->floatsamples, frame->data[0], buffer_size);
            break;

        case AV_SAMPLE_FMT_FLTP:
            frames = (buffer_size >> 2) / channels;
            {
            float *l = (float *)frame->data[0];
            float *r = NULL;
            if (channels == 2)
                r = (float *)frame->data[1];
        ...
        case AV_SAMPLE_FMT_DBLP:
            frames  = (buffer_size >> 3) / channels;
            {
            double *l = (double *)frame->data[0];
            double *r = NULL;
            if (channels == 2)
                r = (double *)frame->data[1];
            float *d = self->floatsamples;
            float *endp = self->floatsamples + (channels * frames);
            while (d < endp)
                {
        ...
        case AV_SAMPLE_FMT_S16:
            ...
            xlplayer_make_audio_to_float(xlplayer, self->floatsamples,
                            frame->data[0], frames, 16, channels);
            break;

        case AV_SAMPLE_FMT_S16P:
            frames = (buffer_size >> 1) / channels;
            {
            int16_t *l = (int16_t *)frame->data[0];
            int16_t *r = NULL;
            if (channels == 2)
                r = (int16_t *)frame->data[1];
            float *d = self->floatsamples;
            float *endp = self->floatsamples + (channels * frames);
            while (d < endp)
                {
        ...
        case AV_SAMPLE_FMT_S32:
            frames = (buffer_size >> 2) / channels;
            xlplayer_make_audio_to_float(xlplayer, self->floatsamples,
                            frame->data[0], frames, 32, channels);
            break;

        case AV_SAMPLE_FMT_S32P:
            frames = (buffer_size >> 2) / channels;
            {
            int32_t *l = (int32_t *)frame->data[0];
            int32_t *r = NULL;
            if (channels == 2)
                r = (int32_t *)frame->data[1];
            float *d = self->floatsamples;
            float *endp = self->floatsamples + (channels * frames);
            while (d < endp)
                {
...
static void avcodecdecode_play(struct xlplayer *xlplayer)
    ...
    do
        ...
        write_frame(xlplayer, self->frame);
